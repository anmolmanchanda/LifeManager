...
class LLMService: ObservableObject {
    ...
    func categorizePARA(input: String) async throws -> PARAProcessingResult {
        print("🔧 LLM: Starting basic PARA categorization")
        
        // Simple fallback categorization
        let result = PARAProcessingResult(
            category: "resource",
            suggestedArea: "General",
            suggestedProject: "Miscellaneous",
            actionableTasks: [],
            tags: [],
            priority: TaskPriority.medium,
            workPersonal: WorkPersonalType.personal,
            confidenceScore: 0.5,
            reasoning: "Basic categorization (LLM disabled for this build)",
            extractedItems: []
        )
        
        print("🔧 LLM: Basic categorization completed")
        return result
    }
