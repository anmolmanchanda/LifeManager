...
class BlobRepository: ObservableObject {
    ...
    func createBlob(
        ...
        return try await supabaseService.insert(blob, into: SupabaseService.TableName.blobs)
    }
    
    /// Fetch all blobs
    func fetchAllBlobs() async throws -> [Blob] {
        return try await supabaseService.fetch(Blob.self, from: SupabaseService.TableName.blobs)
    }
    
    /// Fetch blob by ID
    func fetchBlob(id: UUID) async throws -> Blob? {
        return try await supabaseService.fetchById(Blob.self, from: SupabaseService.TableName.blobs, id: id)
    }
    ...
    func updateBlob(_ blob: Blob) async throws -> Blob {
        return try await supabaseService.update(
            blob,
            in: SupabaseService.TableName.blobs,
            matching: "id",
    ...
    func assignCategory(blobId: UUID, categoryId: UUID, confidenceScore: Double = 0.5) async throws -> BlobCategory {
        ...
        return try await supabaseService.insert(blobCategory, into: SupabaseService.TableName.blobCategories)
    }
    
    /// Assign tag to blob
    func assignTag(blobId: UUID, tagId: UUID) async throws -> BlobTag {
        let blobTag = BlobTag(blobId: blobId, tagId: tagId)
        return try await supabaseService.insert(blobTag, into: SupabaseService.TableName.blobTags)
    }
    ...
    func fetchBlobCategories(blobId: UUID) async throws -> [BlobCategory] {
        let response: [BlobCategory] = try await supabaseService.client
            .from(SupabaseService.TableName.blobCategories)
            .select()
    ...
    func fetchBlobTags(blobId: UUID) async throws -> [BlobTag] {
        let response: [BlobTag] = try await supabaseService.client
            .from(SupabaseService.TableName.blobTags)
            .select()
            .eq("blob_id", value: blobId.uuidString)
            .execute()
            .value
